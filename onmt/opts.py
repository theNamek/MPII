...
def model_opts(parser):
    ...
    """
    # critic
    group = parser.add_argument_group('Model-CRITIC')
    group.add('--critic_src_word_vec_size', '-critic_src_word_vec_size',
              type=int, default=256,
              help='Word embedding size for src.')
    group.add('--critic_enc_layers', '-critic_enc_layers', type=int, default=3)
    group.add('--critic_enc_rnn_size', '-critic_enc_rnn_size', type=int, default=256)
    group.add('--critic_heads', '-critic_heads', type=int, default=8)
    group.add('--critic_transformer_ff', '-critic_transformer_ff', type=int, default=1024)
...
def preprocess_opts(parser):
    ...
    group.add('--num_styles', '-num_styles', type=int, default=2)
    ...
    group.add('--src_vocab_size', '-src_vocab_size', type=int, default=70000,
              help="Size of the source vocabulary")
    group.add('--tgt_vocab_size', '-tgt_vocab_size', type=int, default=70000,
...
def train_opts(parser):
    ...
    # train dec
    group = parser.add_argument_group('train_nli')
    group.add('--max_dec_steps', '-max_dec_steps', type=int, default=40)
    group.add('--show_model_struct', '-show_model_struct', action='store_true')
    group.add('--dynamic_gen_prob', '-dynamic_gen_prob', type=float, default=0.0, help='')
    group.add('--penalty_each_step_cls', '-penalty_each_step_cls', action='store_true')
    group.add('--loss_with_first_cls', '-loss_with_first_cls', action='store_true')
    group.add('--critic_steps', '-critic_steps', type=int, default=10, help='')
    group.add('--ap_critic_steps', '-ap_critic_steps', type=int, default=1, help='')
    group.add('--critic_lr', '-critic_lr', type=float, default=0.00001, help='')
    group.add('--cls_loss_ratio', '-cls_loss_ratio', type=float, default=1.0, help='')
    ...
    group.add('--max_generator_batches', '-max_generator_batches',
              ...
              help="Maximum batches of words in a sequence to run "
                   ...
                   "means: each backward step computes gradients of max_generator_batches tokens")
    ...
    group.add('--adam_beta2', '-adam_beta2', type=float, default=0.9,
              help='The beta2 parameter used by Adam. WMT 0.998'
...
def translate_opts(parser):
    """ NLI options """
    group = parser.add_argument_group('NLI-Translation')
    group.add('--max_dec_steps', '-max_dec_steps', type=int, default=40)

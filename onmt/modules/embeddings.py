...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dropout, dim, max_len=5000):
        ...
        self.dropout = nn.Dropout(p=dropout)
        self.dim = dim
        self.first_pos = nn.Parameter(torch.Tensor(1, 1, dim))
        self.sep_pos = nn.Parameter(torch.Tensor(1, 1, dim))

    def forward(self, emb, step=None, premises_lens=None):
        """Embed inputs.
        ...
        Args:
            ...
            step (int or NoneType): If stepwise (``seq_len = 1``), use
                the encoding for this position.
            premises_lens (list or NoneType): the length of each premise
        """

        emb = emb * math.sqrt(self.dim)
        if step is None:
            if premises_lens is None:
                emb = emb + self.pe[:emb.size(0)]
            else:
                max_len = emb.size(0) - 1
                p_embs = []
                for p_len in premises_lens:
                    h_len = max_len - 1 - p_len
                    cur_emb = [self.first_pos, self.pe[:p_len], self.sep_pos, self.pe[:h_len]]
                    p_embs.append(torch.cat(cur_emb, 0))
                p_emb = torch.cat(p_embs, 1)
                emb = emb + p_emb
        else:
            emb = emb + self.pe[step]
        emb = self.dropout(emb)
...
class Embeddings(nn.Module):
    ...
    def forward(self, source, step=None, premises_lens=None):
        ...
        if self.position_encoding:
            for i, module in enumerate(self.make_embedding._modules.values()):
                if i == len(self.make_embedding._modules.values()) - 1:
                    source = module(source, step=step, premises_lens=premises_lens)
                else:
                    # INFO: UPDATED BY AZ HERE
                    # source = module(source)
                    # [x_len, N, 1] or [x_len, N, vocab_size]
                    sparse_flag = source.size(-1) > 1 and len(source.size()) > 2
                    if sparse_flag:
            ...
            # INFO: UPDATED BY AZ HERE
            # raise ValueError('Not support anymore!')

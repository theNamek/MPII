...
class Trainer(object):
    ...
    def save_results(self, true_batches, normalization, total_stats,
                               report_stats):
        save_file = open('./temp.jsonl', 'at', encoding='utf-8')
        import json
        ...
        for k, batch in enumerate(true_batches):
            ...
            for j in range(1):
                ...
                with torch.cuda.amp.autocast(enabled=self.optim.amp):
                    ...
                    gt_labels = labels_ground_truth.tolist()
                    # [N, y_len, 3]
                    from torch.nn import functional as F
                    raw_probs = F.softmax(torch.cat([x_.unsqueeze(1) for x_ in entail_logits], 1), 2)
                    raw_probs = raw_probs.tolist()
                    # [N, x_len]
                    x_ids = src.squeeze(2).transpose(0, 1)
                    x_sents = [
                        ' '.join([self.train_translator._tgt_vocab.itos[widx] for widx in sent_ids]).replace(' <blank>', '')
                        for sent_ids in x_ids
                    ]
                    y_ids = tgt_outer.squeeze(2).transpose(0, 1).tolist()
                    y_sents = [
                        ' '.join([self.train_translator._tgt_vocab.itos[widx] for widx in sent_ids]).replace(' <blank>', '')
                        for sent_ids in y_ids
                    ]
                    pred_y_ids = torch.argmax(vocab_probs, -1).transpose(0, 1).tolist()
                    pred_y_sents = [
                        ' '.join([self.train_translator._tgt_vocab.itos[widx] for widx in sent_ids]).replace(' <blank>', '')
                        for sent_ids in pred_y_ids
                    ]

                    bz = len(gt_labels)
                    for bzidx in range(bz):
                        save_file.write('{}\n'.format(json.dumps({
                            'x': x_sents[bzidx],
                            'gt_y': y_sents[bzidx],
                            'gt_l': gt_labels[bzidx],
                            'pred_l': pred_labels[bzidx],
                            'pred_y': pred_y_sents[bzidx],
                            'step_l': raw_probs[bzidx],
                        })))
                        save_file.flush()
        save_file.flush()
        save_file.flush()
    ...
    def _gradient_accumulation(self, true_batches, normalization, total_stats,
        ...
        for k, batch in enumerate(true_batches):
            ...
            with torch.no_grad():
                ...
                y_valid_mask = sequence_mask(
                    tgt_lengths.long(), max_len=target_size-1
                ).unsqueeze(2).float()
                tgt_lengths = tgt_lengths.detach()
                y_valid_mask = y_valid_mask.detach()
            for j in range(1):
                # # 1. Create truncated target.
                # tgt = tgt_outer[j: j + trunc_size]
                ...
                with torch.cuda.amp.autocast(enabled=self.optim.amp):
                    ...
                    loss = loss + self.cls_loss_ratio * label_loss
                    if self.loss_with_first_cls:
                        loss = loss + self.cls_loss_ratio * label_loss_first
                try:
                    if loss is not None:
